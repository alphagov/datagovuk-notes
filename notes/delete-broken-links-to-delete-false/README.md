...
The input CSV to filter from is `errors-current.csv`

### Output CSV files after filtering

The file `step1_filter_resources_result.csv` is the output after filtering with:
...
The `step2_filter_by_domains_result.csv` has been filtered to include:
- `domains-to-delete.txt` (domains that indicate a resource is ready for deletion)
- It does also filter:
  - Filter by `to-delete=false`
  - Filter out `http-status=200s`
  - Filter out `deferred orgs`
